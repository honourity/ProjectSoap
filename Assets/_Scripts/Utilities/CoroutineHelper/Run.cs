...
public class Run
{
   ...
   #region Run.AfterFrames
   public static Run AfterFrames(int frames, System.Action aAction)
   {
      var tmp = new Run();
      tmp.action = _RunAfterFrames(tmp, frames, aAction);
      tmp.Start();
      return tmp;
   }
   private static IEnumerator _RunAfterFrames(Run aRun, int frames, System.Action aAction)
   {
      aRun.isDone = false;
   ...
   #region Run.After
   public static Run After(float aDelay, System.Action aAction, bool unscaledTime = false)
   {
      var tmp = new Run();
      tmp.action = _RunAfter(tmp, aDelay, aAction, unscaledTime);
      tmp.Start();
      return tmp;
   }
   private static IEnumerator _RunAfter(Run aRun, float aDelay, System.Action aAction, bool unscaledTime)
   {
      aRun.isDone = false;
      if (!unscaledTime)
   ...
   #region Run.AfterAnimation

   public static Run AfterAnimation(Animator animator, System.Action action)
   {
      var tmp = new Run();
      tmp.action = _RunAfterAnimation(tmp, animator, action);
      tmp.Start();
      return tmp;
   }

   private static IEnumerator _RunAfterAnimation(Run aRun, Animator animator, System.Action action)
   {
      aRun.isDone = false;
      yield return null;
      yield return new WaitUntil(() => animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1f || !animator.gameObject.activeInHierarchy);

      if (!aRun.abort && action != null && animator.gameObject.activeInHierarchy)
         action();
   ...
   public static Run OnDelegate(RunEvent aDelegate, System.Action aAction)
   {
      var tmp = new Run();
   ...
   private static IEnumerator _RunOnDelegate(Run aRun, RunEvent aDelegate, System.Action aAction)
   {
      aRun.isDone = false;
